...
- Phase RV2-B scrap-foraging delivery: `AylaService` autoload (`scripts/delivery/ayla_service.gd`) with scrap submission, sort timer, and scrap-biased `DeliveryConfig` production; `ScrapConfig` model + `data/scrap/scrap_config.json`; `ScrapItem` proximity-E pickup (`scripts/scrapyard/scrap_item.gd` + `scenes/scrapyard/scrap_item.tscn`) using `Interactable3D`; `ScrapyardHud` 5-slot tier hotbar (`scenes/ui/scrapyard_hud.gd` + `scenes/ui/scrapyard_hud.tscn`); `AylaHandoffScreen` (`scripts/scrapyard/ayla_handoff_screen.gd` + `scenes/scrapyard/ayla_handoff_screen.tscn`); Ayla empty-scrap dialogue via `DialogueBox` using authored `yard_empty` lines from `data/routes/routes.json`; loop-scoped `scrap_pool`/`pending_sort` state in `SaveState`/`LoopState`; `ShopController` door path that consumes a ready sort and opens triage. Verified by `tests/delivery/test_ayla_service.gd`, `tests/scrapyard/test_scrap_pickup.gd`, `tests/scrapyard/test_scrapyard_hud_hotbar.gd`, and `tests/scrapyard/test_ayla_dialogue_branch.gd`.
...
- (Phase 3 implemented) Weighted delivery generation, `Marker3D` anchors, full-screen keep/recycle triage, storage-cost inventory enforcement, and the six-state glow legend. The delivery is now earned through scrap-foraging (RV2-B): press-E `ScrapItem` yard pickups, Ayla hand-off/sort, scrap-biased batch, and door-knock triage. Carried scrap is shown in the `ScrapyardHud` 5-slot tier hotbar. Manual on-screen verification of the full forage → hand-off → sort → knock → triage flow is pending.
