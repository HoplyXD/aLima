...
## v3 pacing: 3 real seconds = 1 in-game minute (180 s/hour). Matches DayClock's
## own default — this export exists only for debug speed-ups in the editor.
@export var seconds_per_hour: float = 180.0
...
# --- Maverick's evening stall (v3, story.md §16) --------------------------------
#
# He SELLS now: one overpriced artifact per day (18:00-20:00), and once every
# daily offer is bought, his Day-5 visit adds the fragment for ₱5,000 — replacing
# the old ₱50k last-offer flow (N6). The stall opens after his door dialogue.

var _buyer_shop_screen: BuyerShopScreen


func _open_buyer_shop() -> void:
	if _buyer_shop_screen == null:
		_buyer_shop_screen = BuyerShopScreen.new()
		add_child(_buyer_shop_screen)
		_buyer_shop_screen.closed.connect(_on_buyer_shop_closed)
		_buyer_shop_screen.fragment_bought.connect(_on_buyer_fragment_bought)
	_set_interactables_enabled(false)
	_buyer_shop_screen.open()


func _on_buyer_shop_closed() -> void:
	_set_interactables_enabled(true)
	_refresh_ui()


## The paid fragment is a scripted grant (F1): a direct discovery — the same
## Found -> Portal -> seat chain as a yard find, no hunt in between.
func _on_buyer_fragment_bought() -> void:
	PortalFlowController.flow_finished.connect(_on_buyer_portal_finished, CONNECT_ONE_SHOT)
	FragmentService.release_fragment("fragment_05", "buyer_shop")
	HuntService.mark_found("fragment_05")
	EventBus.fragment_discovered.emit("fragment_05", "")


func _on_buyer_portal_finished(_fragment_id: String) -> void:
	_set_interactables_enabled(true)
	_refresh_ui()
...
func _on_dialogue_finished() -> void:
	...
	if _alya_delivering:
		...
		return
	# Maverick's door dialogue leads into his evening stall (v3: he sells).
	if finished_route_id == "buyer":
		_open_buyer_shop()
		return
