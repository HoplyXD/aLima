...
- Snapshot last audited 2026-06-17; refreshed after the diegetic-presentation pass (visible 3D cleaning tool props, REST-R9 / P4.8; diegetic 3D shop interactables, SHELL-R1/R2 / P4.9) and the Phase 9 journal/fragment-case implementation. The P4.7 focused **3D** restoration view (REST-R8) and the hybrid 2D/3D journal (JRN-R6 / Phase 9) are implemented. Rotatable 3D fragment viewers and per-entry 3D object previews remain Phase 16 polish; the P0 five-slot case and stable entry system are complete. Repository branch is `main`. Refresh this file when implementation materially changes.
...
- GUT 9.6.0 suite passes: `--headless --path . -s addons/gut/gut_cmdln.gd -gdir=res://tests -ginclude_subdirs -gexit` → `295/295 passed` (971 asserts), including Phase 0/1/2 tests, Phase 3 delivery/triage coverage, Phase 4 restoration/opening + 3D-view + tool-prop coverage, the diegetic shop interactable coverage, Phase 5 Spawn Director coverage, Phase 6 Cultural Echoes coverage, Phase 7 cached scanner coverage, Phase 8 portal/Found/Unlock/seating coverage, and Phase 9 journal/Fragment Case coverage (2026-06-17).
- Focused suites pass: `-gdir=res://tests/models` → `11/11 passed`; `-gdir=res://tests/core` → `45/45 passed`; `-gdir=res://tests/delivery` → `33/33 passed` (197 asserts); `-gdir=res://tests/restoration -ginclude_subdirs` → `44/44 passed` (152 asserts; 17 service + 15 view + 7 tool-tray + 5 tool-props-view); `-gdir=res://tests/shop -ginclude_subdirs` → `6/6 passed` (Interactable3D); `-gdir=res://tests/discovery/spawn_director` → `25/25 passed`; `-gdir=res://tests/scanner` → `28/28 passed` (105 asserts); `-gdir=res://tests/journal` → `22/22 passed` (50 asserts); `-gdir=res://tests/portal` → `15/15 passed` (45 asserts).
...
- Each action is reachable two ways: a diegetic 3D prop in the shop (`Interactables/{Door,Workbench,Journal,Phone,Delivery}Interactable`, hover prompt + highlight + click) and a labelled HUD accessibility/fallback button — both fire the same handler. Door opens the dialogue queue (with the visitor). Journal opens the book viewer; Phone shows a "coming soon" placeholder. The Workbench opens the focused **3D** restoration view (`scenes/restoration/restoration_view.tscn`) whose bench shows selectable 3D tool props. "Morning Delivery" triggers one delivery per in-game day and shows a placeholder message if pressed again. Props are disabled while a full-screen overlay is open so clicks don't fall through.
...
- (Phase 4 implemented — P4.8 / REST-R9) **Visible 3D cleaning tool props** (`RestorationToolTray` + `ToolTray` node in `restoration_view.tscn`): one selectable placeholder prop per owned tool, built data-driven; picking a prop (pointer ray or the `restoration_cycle_tool` action) selects the tool via the unchanged `select_tool()` path; the selected prop is highlighted/posed; the HUD tool buttons are demoted to a labelled accessibility/fallback. Presentation-only; never reads carrier identity. Automated verification passes; on-screen verification pending.
- (Implemented — P4.9 / SHELL-R1/R2) **Diegetic 3D shop interactables** (`Interactable3D` + `Interactables/*` in `scenes/Shop.tscn`): door/workbench/journal/phone/delivery are physical props with hover prompt/highlight that fire the existing `ShopController` handlers; HUD buttons remain labelled fallback; props are disabled under full-screen overlays. Placeholder geometry/positioning; final art + on-screen click-through verification pending.
- (Phase 9 implemented) The **hybrid 2D/3D journal** (`BookViewport` + `JournalBook` + `Page` viewports): `BookViewport` pauses via `DayClock.PAUSE_JOURNAL`; page 4 renders the five-slot Fragment Case with placeholder 2D slot art; page 5 shows the object-archive index; pages 6+ show individual `JournalEntry` pages. `JournalService` creates/updates one stable `JournalEntry` per Purple-and-below template on `restoration_completed`, and updates it with `player_verdict` and scanner annotations on `scanner_verdict_committed`. Seated fragments persist across save/load and loop reset. Rotatable 3D fragment viewers and per-entry 3D object previews remain Phase 16 polish; the P0 case/entry system is complete.
...
- Full disposition/evening systems.
...
- `scripts/shop/`: `shop_controller.gd` (orchestration + clock/pause driver + interactable wiring/gating), `interactable_3d.gd` (`Interactable3D`, reusable diegetic 3D shop prop: hover prompt/highlight + `activated` signal; SHELL-R1).
