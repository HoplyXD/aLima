...
##
## Routing rule: Gold finds and the Master Artifact belong to the online museum
## (handled by SeatingService for fragments and the Phase 16 gold flow). Purple-
## and-below restorations are archived here as journal entries. A single entry is
## created on first restoration and updated in place on later restorations and
## scans -- it is never duplicated. Scanner annotations are written to a field
## distinct from the uncle's notes so the two never overwrite each other.
...
func _on_scanner_verdict_committed(instance_id: String, _verdict_name: String) -> void:
	var inst := _find_instance(instance_id)
	if inst == null:
		return
	record_scan(inst)
...
## true when an entry was written. Gold-and-above instances are skipped because
## they route to the museum archive.
...
## Updates the stable journal entry with the latest scanner annotations for the
## instance. The scanner snapshot is advisory; it is stored separately from the
## uncle's notes and never sets a verdict here. Returns true when written.
func record_scan(inst: ObjectInstance) -> bool:
	var template := DataRepository.singleton().get_template(inst.template_id)
	if template == null or not is_journal_rarity(template.base_rarity):
		return false

	var entry := _ensure_entry(inst.template_id, template)
	var record: ScannedRecord = GameState.save_state.persistent.scanned_records.get(inst.template_id)
	if record != null:
...
## template facts seed the entry; mutable progress (condition, annotations) is
## filled by the restoration/scan calls so existing entries are never reset.
