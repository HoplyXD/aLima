...
- **Digital Museum pillar (P16.4/P16.5) verification (2026-07-11):** full recursive GUT suite passes `798/798`; focused `tests/museum/` passes `16/16`; focused `tests/journal/` passes `33/33`; focused `tests/economy/` passes `130/130`; `tests/portal/` passes `17/17`. `server/npm test` passes `33/34` (one pre-existing `negotiate.test.js` fallback-flag failure when no API key is configured); `mock-portal/npm test` passes `11/11`. New/changed GDScript files are `gdformat`/`gdlint` clean. Museum records are source-gated (`source-verification-pending`/`artifact-lock-pending`) and the validator refuses verified-but-placeholder facts.
