...
- `[x]` **RV3.7 Complete UI redesign.** Unified the game's 2D UI under a medieval-archaeology
  "manuscript and brass" design system (`scripts/ui/ui_palette.gd`, `scripts/ui/ui_animations.gd`,
  `Themes/alima_ui.tres`). Restyled title screen, pause menu, dialogue/tutorial boxes, scrapyard HUD,
  storage screen, restoration view, scanner, phone, triage, item inspection, Artifact Found/Portal
  Unlock, quest tracker, journal pages, loading screen, Ayla handoff, destination panel, echo HUD,
  and artifact card. Fixed case-mismatched `Pause_menu.tres` path, duplicate `StateLabel` node in
  `artifact_card.tscn`, and close-animation delays that caused GUT failures. All gameplay logic
  preserved; only visuals, styling, spacing, and animations changed.
...
- `[x]` **Complete UI redesign (2026-07-10):** unified the game's 2D UI under a medieval-archaeology "manuscript and brass" design system (`scripts/ui/ui_palette.gd`, `scripts/ui/ui_animations.gd`, `Themes/alima_ui.tres`). Restyled title screen, pause menu, dialogue/tutorial boxes, scrapyard HUD, storage screen, restoration view, scanner, phone, triage, item inspection, Artifact Found/Portal Unlock, quest tracker, journal pages, loading screen, Ayla handoff, destination panel, echo HUD, and artifact card. Fixed case-mismatched `Pause_menu.tres` path, duplicate `StateLabel` node in `artifact_card.tscn`, and close-animation delays that caused GUT failures. All gameplay logic preserved; only visuals, styling, spacing, and animations changed. Verified on Godot 4.7 stable (2026-07-10): import exit 0; full GUT suite 761/761 passed (2650 asserts); all changed GDScript files are `gdformat`/`gdlint` clean with gdtoolkit 4.5.0. Broader pre-existing formatting/lint debt in untouched files remains.
