...
- `[x]` **P9.1 Build the hybrid 2D/3D journal.** (JRN-R6)
  ...
  - Page 4 (first paper page) shows the five-slot Fragment Case (now with an explicit `N / 5 fragments found` counter); page 5 is the **Condition Guide** (surface conditions grouped by category — Surface Soil / Accretion / Corrosion / Staining / Structural Damage — each with a placeholder colour swatch + name + the tool that treats it, from the `data/journal/surface_conditions.json` catalog via `DataRepository.get_surface_conditions_sorted()`; the `SurfaceCondition` model replaced the old `BlemishType`); page 6 shows the object-archive index; pages 7+ show individual `JournalEntry` pages. (2026-06-18: added the Blemish Guide page + fragment counter; shifted index/entry page numbers accordingly.)
