...
func _render_tools_shop() -> void:
	...
	money.add_theme_font_size_override("font_size", 28)
	...
	ship.add_theme_font_size_override("font_size", 22)
...
func _render_marketplace_offline() -> void:
	...
	notice.add_theme_font_size_override("font_size", 23)
...
func _render_market_home() -> void:
	...
	if sellable.is_empty():
		...
		none.add_theme_font_size_override("font_size", 21)
...
func _make_sell_row(inst: ObjectInstance) -> Control:
	...
	var value_text := (
		"≈₱%d" % MarketplaceService.assessed_value(inst.uid)
		if MarketplaceService.is_scanned(inst.uid)
		else "₱???"
	)
...
func _render_buyer_picker() -> void:
	...
	var title := Label.new()
	title.text = (
		"Who'll buy it? (≈₱%d)" % MarketplaceService.assessed_value(_sell_uid)
...
func _make_buyer_row(persona: BuyerPersona) -> Control:
	...
	occupation.add_theme_font_size_override("font_size", 20)
	...
	offer.add_theme_font_size_override("font_size", 21)
...
func _render_haggle() -> void:
	...
	if persona != null and not persona.occupation.is_empty():
		...
		occ.add_theme_font_size_override("font_size", 21)
	...
	_ai_label = Label.new()
	_ai_label.add_theme_font_size_override("font_size", 20)
	_refresh_ai_label()
	...
	_said_label.add_theme_font_size_override("font_size", 23)
	...
	if _negotiation.is_closed():
		...
		outcome.add_theme_font_size_override("font_size", 24)
		_app_content.add_child(outcome)
		return

	var offer := Label.new()
	offer.text = "Current offer: ₱%d" % _negotiation.current_offer
	offer.add_theme_font_size_override("font_size", 28)
