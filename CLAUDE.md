...
> **STACK CONTRACT:** The project targets **Godot 4.7 (GDScript) + a Node/Express backend**. The presentation is hybrid and trends **diegetic**, and the game runs in **two connected spaces**: a **seated 3D shop interior** (entered through the front door) and a **walkable outdoor scrapyard** (stepped out through that same door) where the player forages rarity-tiered scrap to hand to Ayla — the permanent scrap-hauler/delivery NPC — and tracks hidden fragment carriers by Cultural Echoes. The seated-shop major actions (door, workbench, journal, phone) are **physical 3D interactables** the player hovers and clicks — not flat HUD buttons (SHELL-R1/R3; HUD buttons survive only as labelled accessibility/fallback). **Restoration is a focused 3D object-manipulation interaction** (a manipulable 3D model the player orbits/rotates and cleans by working the tool across its surface), and its **cleaning tools are chosen from a 2D left-edge tool sidebar** (REST-R9) — up to eight numbered rows, each a rotating 3D model of the tool plus the surface conditions it cleans (with cleaning-power numbers) and a durability bar; the **selected tool's 3D model then follows the cursor** while the player works the surface, so the cleaning itself stays a tactile 3D act. The surrounding 2D background + HUD overlay is supportive only (meters, feedback, captions, accessibility); the **journal is hybrid 2D/3D** (a 2D book/paper UI that embeds 3D viewers for the Fragment Case and restored objects); **triage is a diegetic 3D physics interaction** (the player grabs each delivered object and drops it into a Keep box or Recycle bin) with a **2D Control/CanvasLayer accessibility fallback** for keyboard/controller/touch parity; **scanner, dialogue, and Portal** flows remain 2D `Control`/`CanvasLayer` screens. If the project moves to another engine or stack, stop and update §2, §3, §6, the PRD, and the phase tracker together.
...
**B. Fragment lifecycle (revised 2026-07-07, team-lead decision — the hidden-fragment hunt):** `LOCKED` → (story/route completed, persists) → `RELEASED` → (found at its hunt spot + Portal + seated) → `SEATED` (permanent, never spawns again). A `RELEASED` fragment **hides at a Spawn-Director-planned hiding spot** across the walkable spaces (yard, Dump Site, Forbidden Zone, shop interior — `data/delivery/hiding_spots.json`), **re-planned every loop** until found; the player tracks it by Cultural Echoes and **picking it up at the spot IS the discovery** (a bare find — no carrier nesting on the hunt path). Scripted NPC grants (Sam's dirty bag for `fragment_04`) remain valid and keep the clean→open carrier flow. No character hands over a *seated* fragment; the Buyer's qualifying Day-5 last offer releases the fifth fragment into a **guaranteed same-day hunt spot**.

**C. Carrier is a *role*, not an object type** *(scripted-grant and legacy paths)*. When a fragment travels inside an object (an NPC-granted carrier like Sam's bag, or the data-flagged legacy delivery placement `spawn_config.legacy_carrier_delivery`), never model it as a special object: promote an ordinary openable instance (`is_carrier`, `fragment_id`). The 2026-07-07 hunt path places the fragment bare at a hiding spot instead; both paths coexist. See PRD §12.

**D. Two-stage gate (carrier path):** a **carrier** must be **cleaned before it can be opened** — no exceptions on that path; this is what threads NPC-granted fragments through the restoration loop. The hunted **bare find** skips clean→open **by design** (team decision 2026-07-07): its gates are the story release and the echo hunt itself.

**E. Glow legend is fixed.** `white` common · `green` uncommon · `blue` antique · `purple` rare · `gold` historically significant · `flickering` = story/route-connected. **Do not add new glow states.** The hunt find's glint (and any legacy carrier) reuses `flickering`; the **heartbeat audio band** (true-find only) is the disambiguator, not a glow. Flicker only becomes visible at proximity ≥ `GLOW_REVEAL_AT` so the audio leads.
...
**H. Spawn Director guarantees (hunt spots, 2026-07-07):** never plan the same **hiding spot** twice for a player and fragment (per-player `spawn_history`, documented soft-reset to avoid deadlock); never place a fragment somewhere **unreachable that run** — spot availability (location unlocked, Safe code known, day windows like the Dump Site's Days 3–4) is a hard filter, and mid-loop releases only use spots reachable that same day. The legacy `(carrier, container)` never-twice rule still governs the flag-gated carrier-delivery path. See PRD §12.

**I. Echoes run only for a `RELEASED`, unfound fragment hidden in the current walkable space.** The four-band proximity hunt plays only while that fragment is **unfound in this location**; otherwise, silence. The **Heartbeat band is authorized only at the true find** (`EchoController` hunt mode targets exactly the planned spot; on the legacy carrier path it stays gated to `is_carrier == true`) — it must be physically impossible for it to sound anywhere else.
...
**Discovery (the headline mechanic — hidden-fragment hunt, 2026-07-07):**
> [a story completes → the fragment RELEASES and hides at a hunt spot] Hum (far) → Melody (area) → Voice (close) → the find's glint flickers + Heartbeat spikes → **pick it up at the spot** → **Artifact Found** screen → **Portal API** → **Portal Unlock** fact → fragment **seats in journal case** (permanent; the hunt goes quiet). Spots span the scrapyard, Dump Site, Forbidden Zone, and shop interior (availability-gated, never the same spot twice). NPC-scripted grants (Sam's dirty bag) instead hand a **carrier** the player must **clean → open** at the bench before the same Found → seat chain.
...
npm test                        # 15/15 passing as of 2026-07-11 (discovery + museum mirror + auth + persistence)
Pop-Location
