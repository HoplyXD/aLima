...
- Snapshot last audited 2026-06-17; refreshed after the diegetic-presentation pass (visible 3D cleaning tool props, REST-R9 / P4.8; diegetic 3D shop interactables, SHELL-R1/R2 / P4.9). The P4.7 focused **3D** restoration view (REST-R8) remains implemented, the 2D placeholder was retired, and the hybrid 2D/3D journal (JRN-R6, Phase 9) is still docs-ahead-of-code. Repository branch is `main`. Refresh this file when implementation materially changes.
...
- GUT 9.6.0 suite passes: `--headless --path . -s addons/gut/gut_cmdln.gd -gdir=res://tests -ginclude_subdirs -gexit` → `273/273 passed` (921 asserts), including Phase 0/1/2 tests, Phase 3 delivery/triage coverage, Phase 4 restoration/opening + 3D-view + tool-prop coverage, the diegetic shop interactable coverage, Phase 5 Spawn Director coverage, Phase 6 Cultural Echoes coverage, Phase 7 cached scanner coverage, and Phase 8 portal/Found/Unlock/seating coverage (2026-06-17).
- Focused suites pass: `-gdir=res://tests/models` → `11/11 passed`; `-gdir=res://tests/core` → `45/45 passed`; `-gdir=res://tests/delivery` → `33/33 passed` (197 asserts); `-gdir=res://tests/restoration -ginclude_subdirs` → `44/44 passed` (152 asserts; 17 service + 15 view + 7 tool-tray + 5 tool-props-view); `-gdir=res://tests/shop -ginclude_subdirs` → `6/6 passed` (Interactable3D); `-gdir=res://tests/discovery/spawn_director` → `25/25 passed`; `-gdir=res://tests/scanner` → `28/28 passed` (105 asserts).
