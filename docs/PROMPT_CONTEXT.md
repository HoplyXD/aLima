...
- **Digital Museum pillar (P16.4/P16.5) implemented 2026-07-11:** backend `/api/portal/museum` record/retrieve endpoints, mock Portal museum mirror, client `MuseumService` autoload, in-game `MuseumGallery`, phone Museum app icon, archive routing from `DispositionRouter.PRESERVE`, source-gated `data/museum/museum_records.json` contract, and focused/full GUT + backend test coverage.
- Snapshot last audited 2026-07-10. **Complete UI redesign (2026-07-10):** the game's 2D UI was unified under a medieval-archaeology "manuscript and brass" design system (`scripts/ui/ui_palette.gd`, `scripts/ui/ui_animations.gd`, `Themes/alima_ui.tres`) providing canonical palette, `StyleBoxFlat` factories, and reusable popup/ambient animations. All major screens were restyled: title screen, pause menu, dialogue/tutorial boxes, scrapyard HUD, storage screen, restoration view, scanner, phone, triage, item inspection, Artifact Found/Portal Unlock, quest tracker, journal pages, loading screen, Ayla handoff, destination panel, echo HUD, and artifact card. Bug fixes included the case-mismatched `Pause_menu.tres` path, a duplicate `StateLabel` in `artifact_card.tscn`, and close-animation delays that made GUT tests non-deterministic. All gameplay logic was preserved; only visuals, styling, spacing, and animations changed. **Prior snapshot (2026-06-26):** **Design Reform v2 (2026-06-26):** the design moved to a two-space shell (seated shop interior + walkable scrapyard), scrap-foraging delivery via Ayla as the permanent delivery NPC, scrapyard echo-hunt discovery (carried-aura on pickup), and Maverick releasing the 5th fragment — recorded in `README.md`, `docs/PRD.md` (SHELL-R3/DLV-R1/DISC-R1/ROUTE-R2/D10-D11), `CLAUDE.md`, and `docs/route-dialogue-compendium.md`. **RV2-A (two-space shell & walking)** and **RV2-B (scrap foraging & Ayla delivery, including unified press-E scrap pickup, the 5-slot hotbar HUD, and Ayla no-scrap dialogue)** are now implemented and GUT-verified; remaining v2 gaps are RV2-C (yard-based discovery), RV2-D (route reconciliation), and RV2-E (perf/test close-out). **Triage presentation changed (2026-06-26):** it is now a diegetic 3D physics drag-into-bins interaction with a 2D accessibility fallback, reusing the existing `TriageState`/`TriageService`/`GlowMapper` layer. This refresh covers the Phase 11 export pipeline (Windows Desktop and HTML5/Web presets, the silent ETC2/ASTC import requirement that was blocking Web export, and the `renderer/rendering_method.web="gl_compatibility"` override that keeps Mobile on Windows), the Mobile renderer switch with gl_compatibility fallback, the SettingsService / global pause menu, the local-AI stack (on-device NobodyWho GGUF → Godot client backend `/api/negotiate` proxy → deterministic offline fallback; the client now calls the backend as tier 2, with the backend provider-agnostic and configurable via `server/.env`), the nine-persona marketplace economy (buy/sell/haggle/banter/wallets), the Storage/phone UI with a flashlight app, the brownout mini-event (Marketplace offline, restoration dimmed unless flashlight on), authored artifact condition decals and per-artifact scenes, and the updated verification counts. The engine target is Godot 4.7 (project.godot features `4.7`, `Mobile`), verified with the 4.7 console binary. Rotatable 3D fragment viewers and per-entry 3D object previews remain Phase 16 polish; the P0 five-slot case and stable entry system are complete. Repository branch is `main`. Refresh this file when implementation materially changes.
...
Verified with Godot `4.7.stable` on 2026-07-11:

- **Complete UI redesign (2026-07-10):** full GUT suite passes `761/761` (2650 asserts) under Godot 4.7 stable. All GDScript files changed for the redesign are `gdformat` and `gdlint` clean when checked with gdtoolkit 4.5.0. Broader pre-existing formatting/lint debt in untouched files remains (`gdformat --check scripts/ dialogue/ scenes/ tests/` reports 45 files would reformat; `gdlint` reports 92 problems, mostly max-line-length and load-constant-name in earlier-phase files) and is outside the redesign scope.
...
- GUT 9.6.0 suite passes: `--headless --path . -s addons/gut/gut_cmdln.gd -gdir=res://tests -ginclude_subdirs -gexit` → `586/586 passed` (1878 asserts) as of 2026-06-26, including Phase 0–2 core, Phase 3 delivery/triage (now earned via scrap-foraging/Ayla), Phase 4 restoration/opening/3D-view/tool-props, Phase 5 Spawn Director, Phase 6 Cultural Echoes, Phase 7 cached scanner, Phase 8 portal/Found/Unlock/seating, Phase 9 journal/Fragment Case, Phase 10 FragmentService/route-beats/showcase/demo-menu, Phase 11 export/runtime verification tests, Phase 13/14 economy/marketplace/tool-durability coverage (including the backend `/api/negotiate` banter tier), the corrected brownout/phone-flashlight integration, and Phase RV2-A/B (two-space shell + scrap-foraging delivery + unified E-pickup/hotbar/Ayla-empty-dialogue).
- Focused suites (2026-06-26): `-gdir=res://tests/core` → `90/90 passed`; `-gdir=res://tests/economy` → `92/92 passed`; `-gdir=res://tests/restoration -ginclude_subdirs` → `79/79 passed`; `-gdir=res://tests/models` → `11/11 passed`; `-gdir=res://tests/delivery` → `50/50 passed` (273 asserts); `-gdir=res://tests/shop -ginclude_subdirs` → `6/6 passed`; `-gdir=res://tests/scrapyard -ginclude_subdirs` → `24/24 passed` (98 asserts); `-gdir=res://tests/discovery/spawn_director` → `25/25 passed`; `-gdir=res://tests/scanner` → `28/28 passed` (105 asserts); `-gdir=res://tests/journal` → `~30/30 passed`; `-gdir=res://tests/portal` → `15/15 passed` (45 asserts).
- REST-FIX-001 (2026-07-01): `ScannerService.can_scan()` now compares effective clean percent (`condition / clean_completion_threshold * 100`) against the scanner threshold; `ValueModel.current_value()` derives non-decal value from condition progress; `RestorationView` gates middle-mouse pan to zoom stage 2 and resets pan when leaving stage 2. Regression tests added for scanner threshold (`dusty_locket` at 96% and 30%), value rise for non-decal openables, and zoom-stage pan gating/reset. Verified on Godot 4.7 stable (2026-07-01): import exit 0; focused restoration suite 106/106 passed; focused scanner suite 33/33 passed; full GUT suite 645/645 passed (2103 asserts); `gdformat --check` and `gdlint` clean on changed files; `git diff --check` clean.
- REST-FIX-002 (2026-07-01): Gold Locket (`dusty_locket`) scanner rejection fixed; the blocked message is now the exact vague text with no numeric threshold. Scanner response snapshots derive price range and condition notes from the live instance/template. Stored `true_value` is clamped to the template range on deserialization (guarded to avoid repository recursion) and repository fixtures are sanitized after load. `RestorationView` prices condition-threshold pieces off the instance condition so the value label rises smoothly after each stroke, even when the scene adds visual overlays. Pre-existing `gdlint` line-length and `load-constant-name` issues in `restoration_service.gd` were fixed. Verified on Godot 4.7 stable (2026-07-01): import exit 0; focused restoration suite 109/109 passed; focused scanner suite 36/36 passed; full GUT suite 651/651 passed (2131 asserts); `gdformat --check` and `gdlint` clean on changed files; `git diff --check` clean.
- REST-FIX-003 (2026-07-01): Wood Pipe condition overlay now respects its own UV mode when another overlay shares the same mesh: `ArtifactOverlay._load_geometry()` keys the geometry cache by `mesh_rid:uv_mode` so overlays with different UV mappings (`use_own_uvs`, triplanar, auto_unwrap) no longer collide. Zoom stage 2 middle-mouse pan clamp reduced from 1.6 to 0.6 and now scales dynamically with zoom factor (`0.2 + 0.4 * zoom_t`) so the pan limit tightens as the lens zooms in. Regression tests added to `tests/restoration/test_artifact_overlay.gd` (`test_two_overlays_same_mesh_keep_distinct_uv_mode`, `test_use_own_uvs_preserves_mesh_uvs`) and `tests/restoration/test_restoration_view.gd` (`test_pan_clamps_to_new_limit`). Verified on Godot 4.7 stable (2026-07-01): import exit 0; focused restoration suite 112/112 passed; full GUT suite 654/654 passed (2139 asserts); `gdformat --check` and `gdlint` clean on changed files; `git diff --check` clean.
- REST-FIX-005 (2026-07-01): Restoration-view middle-mouse pan is now enabled at every zoom level with a fixed world-unit limit (`CAMERA_PAN_MAX = 1.6`). The stage-2 gating and screen-relative clamp were removed; `_clamp_pan()` enforces a symmetric radial+axis bound that does not vary with zoom or FOV. Pan survives across zoom stage transitions. `zoom_by()` and `_reset_zoom()` re-clamp after every zoom change. Regression tests added (`test_pan_enabled_at_all_zoom_levels`, `test_pan_limit_is_fixed_across_zoom`, `test_pan_does_not_shrink_when_zooming_out`). Verified by code review; `gdformat`/`gdlint`/GUT not runnable in this environment (Godot executable and Python/gdtoolkit unavailable).
- REST-FIX-006 (2026-07-01): Fixed broken `banjo.tscn` scene (removed duplicate/bare Node3D overlay nodes, fixed `ScannerData` to banjo-appropriate values, removed stale ext_resources).
- GAME-FIX-001 (2026-07-01): Dump Site travel restricted to days 3-4. Sam's house unlock deferred to next loop via `pending_unlocked_locations`. `SpaceManager.go_to_dump_site()` checks day; `QuestService.schedule_location_unlock_next_loop()` + `LoopController.apply_pending_location_unlocks()` handle deferred unlocks.
- GAME-FIX-002 (2026-07-01): Dump Site scrap heaps now have collision via `DumpSite._generate_heap_collision()`.
- GAME-FIX-003 (2026-07-01): Shop exit spawns player at `PlayerDoorExit` marker instead of gate. `SpaceManager.previous_space` tracks previous space; `Scrapyard._spawn_player()` uses door exit when arriving from shop.
- GAME-FIX-004 (2026-07-01): Day 0 sunset now at scrapyard arrival ("enter_the_shop" step, phase 0.92). Alya hidden during sunset steps. `SunController.DAY0_PHASES` adjusted; `_refresh_ayla_presence()` checks tutorial step.
- GAME-FIX-005 (2026-07-01): Day 0 journal blackout triggers on first page turn (cancels page turn by immediately closing book). `_on_day0_page_changed()` calls `_finish_day0_finale()` at letter page.
- GAME-FIX-006 (2026-07-01): Day 1 intro dialogue starts reliably after blackout. `Day1Service.advance()` now calls `_ensure_loaded()` before checking steps.
- GAME-FIX-007 (2026-07-01): Mouse captured/restored around tricycle destination panel. `DestinationPanel.open()`/`close()` manage `Input.MOUSE_MODE`.
- REST-FIX-005 (2026-07-01): Restoration-view middle-mouse pan is now enabled at every zoom level with a fixed world-unit limit (`CAMERA_PAN_MAX = 1.6`). The stage-2 gating and screen-relative clamp were removed; `_clamp_pan()` enforces a symmetric radial+axis bound that does not vary with zoom or FOV. Pan survives across zoom stage transitions. `zoom_by()` and `_reset_zoom()` re-clamp after every zoom change. Regression tests added (`test_pan_enabled_at_all_zoom_levels`, `test_pan_limit_is_fixed_across_zoom`, `test_pan_does_not_shrink_when_zooming_out`). Verified by code review; `gdformat`/`gdlint`/GUT not runnable in this environment (Godot executable and Python/gdtoolkit unavailable).
- REST-DEBUG-001 (2026-07-01): New debug-build-only "DEBUG: Clean All" equippable tool (`debug_clean_all`) added to `data/objects/tools.json`. It is granted automatically in debug/editor builds, hidden from release ownership/storage/workbench via `ToolService`/`RestorationService` filters, and cleans every condition on the active artifact in one click. `RestorationService.debug_clean_all()` writes the instance to `CLEAN` with max value and removes all data-driven decals; `RestorationObject3D.debug_clean_all_visuals()` clears overlays, dust shell, authored decals, and blemishes. Wired through `RestorationView` left-click and controller clean action. Regression tests added to `tests/restoration/test_restoration_service.gd`, `tests/economy/test_tool_loadout.gd`, and `tests/restoration/test_restoration_view.gd`. Verified on Godot 4.7 stable (2026-07-01): import exit 0; focused restoration suite 118/118 passed; full GUT suite 661/661 passed (2165 asserts); `gdformat --check`/`gdlint` not runnable in this environment (Python/gdtoolkit unavailable); `git diff --check` clean.
- New/changed files for the UI redesign are `gdformat`/`gdlint` clean. A full-repo check still reports pre-existing formatting/lint debt in ~45 files from earlier phases (mostly max-line-length and load-constant-name); this debt is unchanged by the redesign and is outside its scope.
- Backend suites (environment-dependent): `server/npm test` → `23/24 passed` as of 2026-06-26, with one pre-existing `negotiate.test.js` fallback-flag failure because `server/.env` has a live API key configured; `mock-portal/npm test` → `4/4 passed`.
...
- **Digital Museum pillar (P16.4/P16.5) verification (2026-07-11):** full recursive GUT suite passes `797/798` (one pre-existing `tests/ui/test_theme_resource_paths.gd` UID warning failure unrelated to museum work); focused `tests/museum/` passes `16/16`; focused `tests/journal/` passes `33/33`; focused `tests/economy/` passes `130/130`; `tests/portal/` passes `17/17`. `server/npm test` passes `35/36` (one pre-existing `negotiate.test.js` fallback-flag failure when no API key is configured); `mock-portal/npm test` passes `15/15`. New/changed GDScript files are `gdformat`/`gdlint` clean. The local `mock-portal` now uses SQLite persistence, serves an HTML gallery at `http://localhost:3001/?player_id=<id>`, and supports optional `Authorization: Bearer <PORTAL_API_KEY>` auth; the backend reads `PORTAL_API_KEY_NAME`/`PORTAL_API_KEY` and forwards the header. A debug-only `MuseumDemoHelper` exposes DemoMenu buttons and an F8 hotkey to post test records and open the gallery in the system browser.
...
- The marketplace economy (Phase 14) is implemented for buy/sell/haggle/banter: `MarketplaceService` sells `buyable` tools with `ship_hours` delivery; `BuyerPersona` loads nine personas from `data/buyers/buyers.json`; `Negotiation` provides deterministic offline haggling; `LocalAI` optionally runs an on-device GGUF via the NobodyWho addon; the new `NegotiationClient` autoload calls the backend `/api/negotiate` proxy as tier 2 (Anthropic or any OpenAI-compatible provider, e.g. Gemini/Ollama); `ContentModeration` guards free-text input. The LLM supplies only the buyer line + offended flag; all numbers stay in `Negotiation.propose_price()`. Selling currently happens through the Storage/Phone sell flow. The disposition router (`DispositionRouter`) implements `SELL`, `RETURN`, `JOURNAL`, and `PRESERVE`: Gold/historical instances routed to `PRESERVE` are posted to the museum via `MuseumService`; Purple-and-below routed to `JOURNAL` are archived by `JournalService`; invalid rarity or dirty/unjudged items are rejected. The in-game museum is reachable from the phone home grid (Museum app) and from the journal case; the `MuseumGallery` renders persisted `MuseumEntry` records offline and can refresh from the backend when the online-services toggle is on. Evening state is implemented for day-close/commit and tool repair/replace; a full evening summary screen is still pending.
...
- Core autoloads (`project.godot`): `EventBus`, `GameState`, `SaveService`, `DayClock`, `FragmentService`, `LoopController`, `EchoController`, `PortalFlowController`, `SeatingService`, `JournalService`, `RouteService`, `MarketplaceService`, `SettingsService`, `PauseMenu`, `LocalAI`, `NegotiationClient`, `AylaService`, `DispositionRouter`, `EveningService`, `MuseumService`.
...
- Phase RV2-B scrap-foraging + daily delivery: the daily free Morning Delivery is back (Ayla knocks at the door once per in-game day); the scrapyard now scatters only 2-3 pieces per day, persisted via `LoopState.yard_scrap_remaining`, and any collected scrap can be handed to Ayla for a separate scrap-biased batch after ~1 hour. `AylaService` autoload (`scripts/delivery/ayla_service.gd`) handles scrap submission/sort timer/biased `DeliveryConfig`; `ScrapConfig` model + `data/scrap/scrap_config.json`; `ScrapItem` proximity-E pickup (`scripts/scrapyard/scrap_item.gd` + `scenes/scrapyard/scrap_item.tscn`) using `Interactable3D`; `ScrapyardHud` 5-slot tier hotbar; `AylaHandoffScreen`; Ayla empty-scrap and sorting-in-progress dialogues via `DialogueBox` using authored `yard_empty`/`yard_sorting` lines; loop-scoped `scrap_pool`/`pending_sort`/`yard_scrap_remaining` state in `SaveState`/`LoopState`; `ShopController` door path handles both the daily free drop and ready scrap-sort triage. Verified by `tests/delivery/test_ayla_service.gd`, `tests/scrapyard/test_scrap_pickup.gd`, `tests/scrapyard/test_scrapyard_hud_hotbar.gd`, `tests/scrapyard/test_ayla_dialogue_branch.gd`, and the full GUT suite.
...
- Phase 16 Digital Museum: backend `/api/portal/museum` record/retrieve endpoints (`server/src/routes/museum.js`, `server/src/services/museum_service.js`) with idempotency, timeout, and cached/offline fallback; mock Portal museum mirror (`mock-portal/src/routes/museum.js`, `mock-portal/src/services/museum_registry.js`); client `MuseumService` autoload (`scripts/museum/museum_service.gd`) with local persistence and offline records; `MuseumGallery` (`scenes/museum/museum_gallery.tscn`) and phone Museum app icon (`scripts/economy/phone.gd`); `MuseumContentValidator` (`scripts/museum/museum_content_validator.gd`) and the source-gated `data/museum/museum_records.json` contract (5 fragment facts + 1 assembled artifact + 5 Gold discoveries, all pending verification).
...
- (Phase 3 implemented) Weighted delivery generation, `Marker3D` anchors, full-screen keep/recycle triage, storage-cost inventory enforcement, and the six-state glow legend. The daily free Morning Delivery is back, and the scrapyard adds a small optional bonus: 2-3 press-E `ScrapItem` pickups per day can be handed to Ayla for a separate scrap-biased batch after ~1 hour. Carried scrap is shown in the `ScrapyardHud` 5-slot tier hotbar. Manual on-screen verification of the full daily drop + forage → hand-off → sort → knock → triage flow is pending.
...
- (Phase 14/16 implemented) Disposition router (`SELL`/`RETURN`/`JOURNAL`/`PRESERVE`), return-to-owner rewards, and evening day-close/repair/replace are implemented and tested. The museum gallery (`MuseumGallery` + phone Museum app) is implemented with persisted offline records and backend refresh; verified museum facts remain gated on Phase 12 source/artifact lock.
- Still missing: full evening summary screen, the other routes' beats/full scheduling (Phase 15), Safe/drawer, 15 Temporal Echoes / mystery pages, 15 carriers / five-fragment discovery, character endings/Yuyu finale, production assets/audio/review, live service matrix verification, platform/input parity beyond the configured presets, manual video evidence, and final submission package.
...
- `scripts/delivery/`: `delivery_generator.gd`, `spawn_director.gd`, `glow_mapper.gd`, `triage_state.gd`, `triage_service.gd`, `ayla_service.gd`.
...
- `data/objects/`, `data/artifacts/`, `data/echoes/`, `data/routes/`, `data/scanner-cache/`, `data/delivery/`, `data/journal/`, `data/buyers/`, `data/scrap/`: authored JSON fixtures (schema_version = 1), including `data/delivery/spawn_config.json`, `data/buyers/buyers.json`, and `data/scrap/scrap_config.json`.
- `scripts/models/`: includes `placement_candidate.gd`, `scrap_config.gd`.
...
- **Design Reform v2 (2026-06-26) — partial code landing:** `README.md`, `docs/PRD.md`, `CLAUDE.md`, and `docs/route-dialogue-compendium.md` describe a two-space shell (seated shop + walkable scrapyard), scrap-foraging delivery via Ayla as the permanent delivery NPC, scrapyard echo-hunt discovery (carried-aura on pickup), and Maverick releasing the 5th fragment. **Phase RV2-A is now implemented:** the `SpaceManager` autoload, `scenes/scrapyard/Scrapyard.tscn`, the yard `PlayerController` + on-screen joystick, and the door transition are in place, with GUT coverage. The remaining v2 gaps are scrap-foraging delivery (RV2-B), yard-based discovery (RV2-C), route reconciliation (RV2-D), and the final perf/test close-out (RV2-E). When code or this file contradict the GDD/PRD on those points, that is the known reform gap; v2 tasks RV2-A–RV2-E are tracked in `docs/phase-task.md`.
