...
- `[x]` **P9.1 Build the hybrid 2D/3D journal.** (JRN-R6)
  ...
  - Page 4 (first paper page) shows the five-slot Fragment Case (now with an explicit `N / 5 fragments found` counter); page 5 is the **Blemish Guide** (each blemish: placeholder colour swatch + name + the tool that cleans it, from the `data/journal/blemishes.json` catalog via `DataRepository.get_blemish_types_sorted()`); page 6 shows the object-archive index; pages 7+ show individual `JournalEntry` pages. (2026-06-18: added the Blemish Guide page + fragment counter; shifted index/entry page numbers accordingly.)
...
- `[ ]` **P14.1 Build listing and negotiation state.** Connect condition, authenticity verdict, description honesty, persona preference, budget, offers, acceptance, rejection, and walk-away outcomes.
  - Progress (2026-06-18): the **phone Marketplace BUY side** logic + screen exist. `MarketplaceService` (autoload) lists `buyable` tools, spends `loop.money`, and schedules shipments that arrive after `ship_hours` of in-game time (delivered on the hour tick into `loop.owned_tools`). The shop **Phone interactable** now opens `MarketplaceScreen` (`scenes/ui/marketplace_screen.tscn`, `DayClock.PAUSE_PHONE`) with a per-tool Buy button; covered by `tests/economy/test_marketplace.gd`, `test_marketplace_screen.gd`, and the shop smoke `test_phone_opens_marketplace_and_pauses`. The **≤10 bench loadout + restore-target** selection (`ToolService`, `loop.workbench_tools`/`restore_target_uid`) is the Storage-button backend (`test_tool_loadout.gd`); its on-screen Storage UI and wiring the bench to show only the loadout are still to do. **Selling restored artifacts and the online buyer banter (six-persona server-side LLM negotiation) are deliberately not implemented** — that is the core of this task and remains to do. On-screen verification of the phone → Marketplace flow is a pending manual gate.
