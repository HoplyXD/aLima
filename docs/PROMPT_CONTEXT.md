...
- New/changed RV2-B files are gdformat/gdlint clean. `gdformat --check scripts scenes dialogue tests` still reports pre-existing formatting drift in ~19 unrelated files from earlier phases; `gdlint scripts scenes dialogue tests` reports no new problems.
