...
- GUT 9.6.0 suite passes: `--headless --path . -s addons/gut/gut_cmdln.gd -gdir=res://tests -ginclude_subdirs -gexit` → `508/508 passed` (1638 asserts) as of 2026-06-23, including Phase 0–2 core, Phase 3 delivery/triage, Phase 4 restoration/opening/3D-view/tool-props, Phase 5 Spawn Director, Phase 6 Cultural Echoes, Phase 7 cached scanner, Phase 8 portal/Found/Unlock/seating, Phase 9 journal/Fragment Case, Phase 10 FragmentService/route-beats/showcase/demo-menu, Phase 11 export/runtime verification tests, Phase 13/14 economy/marketplace/tool-durability coverage, and the corrected brownout/phone-flashlight integration.
- Focused suites (2026-06-23): `-gdir=res://tests/core` → `90/90 passed`; `-gdir=res://tests/economy` → `88/88 passed`; `-gdir=res://tests/restoration -ginclude_subdirs` → `79/79 passed`; `-gdir=res://tests/models` → `11/11 passed`; `-gdir=res://tests/delivery` → `~33/33 passed` (197 asserts); `-gdir=res://tests/shop -ginclude_subdirs` → `6/6 passed`; `-gdir=res://tests/discovery/spawn_director` → `25/25 passed`; `-gdir=res://tests/scanner` → `28/28 passed` (105 asserts); `-gdir=res://tests/journal` → `~30/30 passed`; `-gdir=res://tests/portal` → `15/15 passed` (45 asserts).
- `gdformat --check scripts scenes dialogue tests` reports 155 files would be left unchanged; `gdlint scripts scenes dialogue tests` reports no problems.
...
- Core autoloads (`project.godot`): `EventBus`, `GameState`, `SaveService`, `DayClock`, `FragmentService`, `LoopController`, `EchoController`, `PortalFlowController`, `SeatingService`, `JournalService`, `RouteService`, `MarketplaceService`, `SettingsService`, `PauseMenu`, `LocalAI`.
